...
subroutine compute_interp_rho
    ...
    real(amrex_real), contiguous, dimension(:,:,:,:), pointer :: RHOxptr, RHOyptr, RHOzptr, phiborderptr
    integer :: nlevs
    ...
    do ilev = 0, nlevs
	  ...
	  fzhi = ubound(RHOzptr)
          ...
          call interp_rho(ilev, bx%lo, bx%hi, RHOxptr,&
  	       fxlo,fxhi,RHOyptr,fylo,fyhi,RHOzptr,fzlo,fzhi,phiborderptr(:,:,:,1),borlo,borhi,amrex_geom(ilev)%dx, amrex_problo)
...
subroutine interp_rho(level, lo, hi, RHOx, fxlo, fxhi, RHOy, fylo, fyhi, RHOz, fzlo, fzhi, RHO, borlo, borhi, dx, prob_lo)
