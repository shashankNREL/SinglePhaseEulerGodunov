...
subroutine vortmag(level, lo, hi, phiborder, borlo, borhi, vortmagnitude, clo, chi, dx, prob_lo)
  ...
  double precision, dimension(borlo(1):borhi(1),borlo(2):borhi(2),borlo(3):borhi(3),ncomp) :: phiborder
  ...
  do k=borlo(3),borhi(3)
     do j=borlo(2),borhi(2)
        do i=borlo(1),borhi(1)
   	   U(i,j,k)=phiborder(i,j,k,2)/phiborder(i,j,k,1)	 
   	   V(i,j,k)=phiborder(i,j,k,3)/phiborder(i,j,k,1)	 
   	   W(i,j,k)=phiborder(i,j,k,4)/phiborder(i,j,k,1)	 
  	end do
     end do
  end do	
