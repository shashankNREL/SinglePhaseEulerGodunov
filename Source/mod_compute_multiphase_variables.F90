...
subroutine compute_multiphase_variables(phimfab,phi_mpmfab)
    ...
    do ilev = 0, nlevs
	  ...
	  chi = ubound(phiptr)
          ...
          call multiphase_variables(ilev, bx%lo, bx%hi, phiptr(:,:,:,1), phiptr(:,:,:,2), phiptr(:,:,:,3), phiptr(:,:,:,4), phiptr(:,:,:,5), clo, chi, &
...
subroutine multiphase_variables (level, lo, hi, Grho, GrhoU, GrhoV, GrhoW, GrhoE, clo, chi, RHO, rhoU, rhoV, rhoW, rhoE, dx, prob_lo)

  implicit none
  integer, intent(in) :: level, lo(3), hi(3), clo(3), chi(3)
  double precision, dimension(clo(1):chi(1),clo(2):chi(2),clo(3):chi(3)) :: Grho, GrhoU, GrhoV, GrhoW, GrhoE
  ...
  do k=lo(3),hi(3)
     do j=lo(2),hi(2)
        do i=lo(1),hi(1)
	   RHO(i,j,k) = Grho(i,j,k)
	   rhoU(i,j,k) = GrhoU(i,j,k)
	   rhoV(i,j,k) = GrhoV(i,j,k)
	   rhoW(i,j,k) = GrhoW(i,j,k)
	   rhoE(i,j,k) = GrhoE(i,j,k)
        end do
     end do
  end do
