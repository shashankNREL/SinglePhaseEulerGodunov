...
subroutine compute_correct_energy
    ...
    type(amrex_box) :: bx
    type(amrex_mfiter) :: mfi
    real(amrex_real), contiguous, dimension(:,:,:,:), pointer :: phiptr, PXptr, Ufaceptr, PYptr, Vfaceptr, PZptr, Wfaceptr
    integer :: nlevs
    ...
    do ilev = 0, nlevs
	  ...
	  chi = ubound(phiptr)
          ...
          call correct_energy(ilev, bx%lo, bx%hi, phiptr(:,:,:,5), clo, chi, PXptr, Ufaceptr, fxlo, fxhi, &
...
subroutine correct_energy (level, lo, hi, GrhoE, clo, chi, PX, Uface, fxlo, fxhi,& 
							   PY, Vface, fylo, fyhi,&
							   PZ, Wface, fzlo, fzhi,&
							   dx, prob_lo)
  ...
  double precision, dimension(clo(1):chi(1),clo(2):chi(2),clo(3):chi(3)) :: GrhoE
  ...
  do k=lo(3),hi(3)
     do j=lo(2),hi(2)
        do i=lo(1),hi(1)
                GrhoE(i,j,k) = GrhoE(i,j,k)-dt_step*( (PX(i+1,j,k)*Uface(i+1,j,k)-PX(i,j,k)*Uface(i,j,k))/dx(1)+&
    ...
    do ilev = 0, nlevs
       ...
       do while (mfi%next())
          ...
          call update_cons_momentum(ilev, bx%lo, bx%hi, phi_mpptr(:,:,:,1), phi_mpptr(:,:,:,2), phi_mpptr(:,:,:,3),phi_mpptr(:,:,:,4),clo, chi, &
				    phiptr(:,:,:,1), phiptr(:,:,:,2), phiptr(:,:,:,3), phiptr(:,:,:,4), amrex_geom(ilev)%dx, amrex_problo)
...
subroutine update_cons_momentum(level,lo,hi,RHO,rhoU,rhoV,rhoW,clo,chi,Grho,GrhoU,GrhoV,GrhoW,dx,prob_lo)
   integer :: level, i, j, k
   ...
   double precision, intent(inout), dimension(clo(1):chi(1),clo(2):chi(2),clo(3):chi(3)) :: Grho,GrhoU,GrhoV,GrhoW
