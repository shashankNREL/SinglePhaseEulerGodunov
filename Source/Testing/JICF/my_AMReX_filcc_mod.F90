...
contains
  ...
  subroutine my_filccn(compno,lo, hi, q, q_lo, q_hi, ncomp, domlo, domhi, dx, xlo, bc)
    ...
    real(amrex_real) :: Tinfty, Tj, Vj, Mj, rhoj, Vz, afac, rad, jetrad, Grho, GrhoU, GrhoW, GrhoE
       ...
       if (lo(3) < klo) then
        ...
        do k = kmin, kmax
           do j = lo(2), hi(2)
		      ...
		      Grho = 1.176d0!rhoj*(0.5d0*0.4d0*Vz*(1.0d0-Vz)*Mj**2+Vz+Tinfty/Tj*(1.0d0-Vz))**(-1)
                      GrhoW = 1.176d0*40.0d0
                      GrhoE = 101325.0/0.4d0 + 0.5d0*GrhoW**2/Grho
                      ...
                      q(i,j,k,n) = 2.0d0*Grho - q(i,j,klo,n)
                        ...
                        elseif(compno.eq.5)then
                      q(i,j,k,n) = 2.0d0*(101325.0d0/0.4d0+0.5d0*GrhoW**2/Grho) - q(i,j,klo,n)
                        else
