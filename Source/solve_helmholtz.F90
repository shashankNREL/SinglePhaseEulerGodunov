...
contains  
  ...
  subroutine solve_abeclaplacian ()
    ...
    do idim=1,amrex_spacedim
  	...
  	nodal = .false.
        nodal(idim) = .true.
	call amrex_multifab_build(betatemp(idim,ilev), phi_new(ilev)%ba, phi_new(ilev)%dm, 1, 0,nodal)
      enddo
    enddo

    do idim=1,amrex_spacedim
      do ilev = 0, nlevs
	call betatemp(idim,ilev)%copy(beta(idim,ilev), 1, 1, 1, 0)
      enddo
    enddo

    do idim=1,amrex_spacedim
      do ilev = 0, nlevs
	call betatemp(idim,ilev)%multiply(betatemp(idim,ilev), 1, 1, 1, 0)
      enddo
    enddo

    do idim=1,amrex_spacedim
      do ilev = 0, nlevs
	call beta(idim,ilev)%divide(betatemp(idim,ilev), 1, 1, 1, 0)
      enddo
    enddo

    do ilev=0,nlevs
       do idim = 1, amrex_spacedim
          call amrex_multifab_destroy(betatemp(idim,ilev))
       enddo
    enddo
